...
import { Container, Row, Card, Accordion, Table, Collapse, ListGroup, Spinner } from "react-bootstrap";
...
function MatchResults() {
  ...
  if (matchResults.length === 0) {
    return (
      <Container fluid="md" className="d-flex justify-content-center match-container-1 mt-4 w-50">
      ...
      <div style={{ width: "100%", maxWidth: "800px" }}>
        <Container>
          <Row className="justify-content-center mt-3 mb-3">
            <div className="text-white py-2 align-items-center" style={{ border: "1px solid #942cd2", backgroundColor: "#942cd2", borderRadius: "5px" }}>
            ...
            <Accordion defaultActiveKey="0" style={{ width: "100%", padding: "0" }}>
              {matchResults.map((matchResult, index) => (
                <Accordion.Item eventKey={index.toString()} key={index}>
                  <Accordion.Header>
                    <div className="d-flex w-100 justify-content-between align-items-center flex-nowrap" style={{ fontSize: "0.9rem" }}>
                      <div>
                                    ...
                                    <tbody>
                                      {Object.entries(match.bm25_scores).length > 0 ? (
                                        Object.entries(match.bm25_scores).map(([key, value]) => (
                                          <tr key={key}>
                                            <td>{key}</td>
                                            <td style={{ color: value > 0 ? "green" : "inherit" }}>
                                              {value} {value === 0 && "(No matches)"}
                                            </td>
                                          </tr>
                                        ))
                                      ) : (
                                        <tr>
                                          <td colSpan={2}>No BM25 scores available</td>
                                        </tr>
                                      )}
                                    </tbody>
                                  </Table>
